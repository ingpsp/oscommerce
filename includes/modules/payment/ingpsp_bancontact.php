<?php
$can_activate = false;
// determine if we can do Bancontact
...
class ingpsp_bancontact {
  ...
  // Class Constructor
  function ingpsp_bancontact() {
    global $order;

    $this->code = 'ingpsp_bancontact';
    $this->title_selection = MODULE_PAYMENT_INGPSP_BANCONTACT_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_bancontact';
    } else if ( $payment=='ingpsp_bancontact' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_BANCONTACT_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_BANCONTACT_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $ingpsp_order = $this->ingpsp->ingCreateBcOrder( $insert_id, $order->info['total'], STORE_NAME . " " . $insert_id, tep_href_link( "ext/modules/payment/ingpsp/redir.php", '', 'SSL' ), $customer );
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_BANCONTACT_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable ING PSP Bancontact Module',
