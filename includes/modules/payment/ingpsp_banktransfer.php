<?php
$can_activate = false;
// determine if we can do Banktransfer
...
if (strlen(MODULE_PAYMENT_INGPSP_APIKEY)) {
  ...
  if ( file_exists($ing_lib_location)) {
    ...
    if (in_array('banktransfer', $allowed_products))
      $can_activate = true;
  }
}
...
class ingpsp_banktransfer {
  ...
  // Class Constructor
  function ingpsp_banktransfer() {
    global $order;

    $this->code = 'ingpsp_banktransfer';
    $this->title_selection = MODULE_PAYMENT_INGPSP_BANKTRANSFER_TEXT_TITLE;
    $this->title = 'ING PSP ' . $this->title_selection;
    $this->description = MODULE_PAYMENT_INGPSP_BANKTRANSFER_TEXT_DESCRIPTION;
    $this->email_footer = MODULE_PAYMENT_INGPSP_BANKTRANSFER_TEXT_EMAIL_FOOTER;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_banktransfer';
    } else if ( $payment=='ingpsp_banktransfer' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_BANKTRANSFER_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_BANKTRANSFER_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function confirmation() {
    return array('title' => MODULE_PAYMENT_INGPSP_BANKTRANSFER_TEXT_DESCRIPTION);
  }
  ...
  function after_process() {
    ...
    $this->email_footer = sprintf(MODULE_PAYMENT_INGPSP_BANKTRANSFER_TEXT_EMAIL_FOOTER, $order->info['total'], $ingpsp_order['transactions'][0]['payment_method_details']['reference']);
    ...
    $email_order = str_replace('[[REFERENCE]]', $ingpsp_order['transactions'][0]['payment_method_details']['reference'], $email_order);
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_BANKTRANSFER_STATUS'" );
  ...
  function install() {
    ...
    // ADD THE INGPSP BANK REFERENCE TO THE ORDERS TABLE
    if (!$this->tableColumnExists("orders", "ingpsp_bank_reference")) {
      if (!tep_db_query("ALTER TABLE orders ADD ingpsp_bank_reference VARCHAR( 16 ) NULL DEFAULT NULL ;")) {
        die("To be able to work; please add the column ingpsp_bank_reference (VARCHAR 16, DEFAULT NULL) to your order table");
      }
    }    

    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable ING PSP Banktransfer Module',
