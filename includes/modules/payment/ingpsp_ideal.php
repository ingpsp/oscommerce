...
if (strlen(MODULE_PAYMENT_INGPSP_APIKEY)) {
  ...
  if ( file_exists($ing_lib_location)) {
    ...
    if (in_array('ideal', $allowed_products))
      $can_activate = true;
  }
}
...
class ingpsp_ideal {
  ...
  // Class Constructor
  function ingpsp_ideal() {
    global $order;

    $this->code = 'ingpsp_ideal';
    $this->title_selection = MODULE_PAYMENT_INGPSP_IDEAL_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_ideal';
    } else if ( $payment=='ingpsp_ideal' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
    ...
    $this->email_footer = MODULE_PAYMENT_INGPSP_IDEAL_TEXT_EMAIL_FOOTER;
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_IDEAL_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_IDEAL_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function javascript_validation() {
    $js = 'if (payment_value == "' . $this->code . '") {' . "\n" .
      '  var ingpsp_issuer_id = document.checkout_payment.ingpsp_issuer_id.value;' . "\n" .
      '  if (ingpsp_issuer_id == "") {' . "\n" .
      '    error_message = error_message + "' . MODULE_PAYMENT_INGPSP_IDEAL_TEXT_SELECT_BANK . '";' . "\n" .
      '    error = 1;' . "\n" .
      '  }' . "\n" .
      '}' . "\n";
    return $js;
  ...
  function get_issuers() {
    $issuers_tmp = $this->ingpsp->ingGetIssuers();
    ...
    if ( is_array( $issuers_tmp ) && !isset( $issuers_tmp['error'] ) ) {
      ...
      $i = 0;
      $issuers_array[$i]['id'] = '';
      $issuers_array[$i]['text'] = MODULE_PAYMENT_INGPSP_IDEAL_TEXT_SELECT_BANK;
  ...
  function pre_confirmation_check() {
    $_SESSION['ingpsp_issuer_id'] = $_POST['ingpsp_issuer_id'];
  }
  ...
  function after_process() {
    ...
    $ingpsp_order = $this->ingpsp->ingCreateIdealOrder( $insert_id, $order->info['total'], $_SESSION['ingpsp_issuer_id'], tep_href_link( "ext/modules/payment/ingpsp/redir.php", '', 'SSL' ), STORE_NAME . " " . $insert_id, $customer );
    ...
    if ( !is_array( $ingpsp_order ) or array_key_exists( 'error', $ingpsp_order) or $ingpsp_order['status'] == 'error' ) {
      ...
      tep_redirect( $ingpsp_order['transactions'][0]['payment_url'] );
    }
    return false;
  }
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_IDEAL_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable ING PSP iDEAL Module',
