...
class ingpsp_paypal {
  ...
  // Class Constructor
  function ingpsp_paypal() {
    global $order;

    $this->code = 'ingpsp_paypal';
    $this->title_selection = MODULE_PAYMENT_INGPSP_PAYPAL_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_paypal';
    } else if ( $payment=='ingpsp_paypal' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_PAYPAL_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_PAYPAL_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $ingpsp_order = $this->ingpsp->ingCreatePaypalOrder( $insert_id, $order->info['total'], STORE_NAME . " " . $insert_id, tep_href_link( "ext/modules/payment/ingpsp/redir.php", '', 'SSL' ), $customer );
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_PAYPAL_STATUS'" );
