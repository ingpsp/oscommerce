<?php
$can_activate = false;
// determine if we can do Payconiq
...
class ingpsp_payconiq {
  ...
  // Class Constructor
  function ingpsp_payconiq() {
    global $order;

    $this->code = 'ingpsp_payconiq';
    $this->title_selection = MODULE_PAYMENT_INGPSP_PAYCONIQ_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_payconiq';
    } else if ( $payment=='ingpsp_payconiq' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_PAYCONIQ_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_PAYCONIQ_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_PAYCONIQ_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable ING PSP Payconiq Module',
