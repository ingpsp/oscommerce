<?php
$can_activate = false;
// determine if we can do Credit card
...
class ingpsp_creditcard {
  ...
  // Class Constructor
  function ingpsp_creditcard() {
    global $order;

    $this->code = 'ingpsp_creditcard';
    $this->title_selection = MODULE_PAYMENT_INGPSP_CREDITCARD_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_INGPSP_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'ingpsp_creditcard';
    } else if ( $payment=='ingpsp_creditcard' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_INGPSP_CREDITCARD_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_INGPSP_CREDITCARD_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $ingpsp_order = $this->ingpsp->ingCreateCreditcardOrder( $insert_id, $order->info['total'], tep_href_link( "ext/modules/payment/ingpsp/redir.php", '', 'SSL' ), STORE_NAME . " " . $insert_id, $customer );
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INGPSP_CREDITCARD_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable ING PSP Creditcard Module',
